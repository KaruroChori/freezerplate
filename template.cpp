...
#endif
    out.write((const char*)dtmp.data(),dtmp.size());
} auto proj_name = env.child("project").attribute("name").as_string("undefined"); {constexpr const uint8_t tmp[] = {0X1,0X0,0X0,0X0,0X78,0X9C,0X1,0X1,0X0,0XFE,0XFF,0XA,0X0,0XB,0X0,0XB,};
#ifdef TE4_COMPRESS
    auto dtmp = inflate({tmp,sizeof(tmp)});
#else
    auto dtmp = std::basic_string_view<uint8_t>(tmp,sizeof(tmp));
#endif
    out.write((const char*)dtmp.data(),dtmp.size());
} auto proj_version = env.child("project").attribute("version").as_string("0.1.0"); {constexpr const uint8_t tmp[] = {0X10,0X0,0X0,0X0,0X78,0X9C,0X1,0X10,0X0,0XEF,0XFF,0XA,0XA,0X70,0X72,0X6F,0X6A,0X65,0X63,0X74,0X28,0XA,0X20,0X20,0X20,0X20,0X27,0X25,0X3A,0X3,0XE5,};
#ifdef TE4_COMPRESS
    auto dtmp = inflate({tmp,sizeof(tmp)});
#else
    auto dtmp = std::basic_string_view<uint8_t>(tmp,sizeof(tmp));
...
#endif
    out.write((const char*)dtmp.data(),dtmp.size());
}WRITE_MESON(proj_name){constexpr const uint8_t tmp[] = {0X4,0X0,0X0,0X0,0X78,0X9C,0X1,0X4,0X0,0XFB,0XFF,0X27,0X2C,0XA,0X29,0X1,0X61,0X0,0X87,};
#ifdef TE4_COMPRESS
    auto dtmp = inflate({tmp,sizeof(tmp)});
#else
    auto dtmp = std::basic_string_view<uint8_t>(tmp,sizeof(tmp));
