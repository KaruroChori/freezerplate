...
struct init_t{ init_t(){fs_tree.emplace("platforms/default.ini",writer_file_0);} } static init;static constexpr int perms = 436;static constexpr const char* name  = "default.ini";
...
static constexpr int perms = 509;struct init_t{ init_t(){fs_tree.emplace("platforms",writer_dir_1);} } static init;static constexpr const char* name  = "platforms";
fs::path file = dir / (_name.length()!=0?_name.c_str():name);
...
struct init_t{ init_t(){fs_tree.emplace("README.md",writer_file_1);} } static init;static constexpr int perms = 436;static constexpr const char* name  = "README.md";
fs::path file = dir / (_name.length()!=0?_name.c_str():name);
...
for (pugi::xml_node child: proj_description.children()){
    if (child.type() == pugi::node_pcdata)
        WRITE(child.value());
}{constexpr const uint8_t tmp[] = {0XA,0XA,0X23,0X23,0X20,0X48,0X6F,0X77,0X20,0X74,0X6F,0X20,0X63,0X6F,0X6D,0X70,0X69,0X6C,0X65,0XA,0XA,0X23,0X23,0X20,0X48,0X6F,0X77,0X20,0X74,0X6F,0X20,0X75,0X73,0X65,0XA,0XA,0X23,0X23,0X20,0X4C,0X69,0X63,0X65,0X6E,0X63,0X65,0XA,};
#ifdef TE4_COMPRESS
    auto dtmp = inflate({tmp,sizeof(tmp)});
#else
    auto dtmp = std::basic_string_view<uint8_t>(tmp,sizeof(tmp));
...
struct init_t{ init_t(){fs_tree.emplace("meson.build",writer_file_2);} } static init;static constexpr int perms = 436;static constexpr const char* name  = "meson.build";
fs::path file = dir / (_name.length()!=0?_name.c_str():name);
...
struct init_t{ init_t(){fs_tree.emplace(".gitignore",writer_file_3);} } static init;static constexpr int perms = 436;static constexpr const char* name  = ".gitignore";
fs::path file = dir / (_name.length()!=0?_name.c_str():name);
...
struct init_t{ init_t(){fs_tree.emplace("build.sh",writer_file_4);} } static init;static constexpr int perms = 509;static constexpr const char* name  = "build.sh";
fs::path file = dir / (_name.length()!=0?_name.c_str():name);
...
struct init_t{ init_t(){fs_tree.emplace("LICENCE.md",writer_file_5);} } static init;static constexpr int perms = 436;static constexpr const char* name  = "LICENCE.md";
fs::path file = dir / (_name.length()!=0?_name.c_str():name);
...
struct init_t{ init_t(){fs_tree.emplace("subprojects/pugixml.wrap",writer_file_6);} } static init;static constexpr int perms = 436;static constexpr const char* name  = "pugixml.wrap";
fs::path file = dir / (_name.length()!=0?_name.c_str():name);
...
struct init_t{ init_t(){fs_tree.emplace("subprojects/miniz.wrap",writer_file_7);} } static init;static constexpr int perms = 436;static constexpr const char* name  = "miniz.wrap";
fs::path file = dir / (_name.length()!=0?_name.c_str():name);
...
static constexpr int perms = 509;struct init_t{ init_t(){fs_tree.emplace("subprojects",writer_dir_2);} } static init;static constexpr const char* name  = "subprojects";
fs::path file = dir / (_name.length()!=0?_name.c_str():name);
fs::create_directories(file);
...
struct init_t{ init_t(){fs_tree.emplace("meson.options",writer_file_8);} } static init;static constexpr int perms = 436;static constexpr const char* name  = "meson.options";
fs::path file = dir / (_name.length()!=0?_name.c_str():name);
