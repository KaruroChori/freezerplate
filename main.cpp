...
generate_file_function(ctx_t &ctx, const fs::path &source) {
  ...
  for (; offset < buffer.size();) {
    if (mode == 0 &&
        ((buffer.size() - offset >= ctx.LBRACE_len &&
          memcmp(buffer.data() + offset, ctx.LBRACE, ctx.LBRACE_len) == 0) ||
    ...
    out.write((const char*)dtmp.data(),dtmp.size());
      ...
      if (offset < buffer.size())
        offset += ctx.LBRACE_len;
      else
        break;
      base = offset;
      continue;
    } else if (mode == 1 && ((buffer.size() - offset >= ctx.RBRACE_len &&
                              memcmp(buffer.data() + offset, ctx.RBRACE,
                                     ctx.RBRACE_len) == 0) ||
                             offset == buffer.size())) {
      // To get the last one
      if (offset == buffer.size() - 1)
      ...
      mode = 0;
      if (offset < buffer.size()) {
        offset += ctx.RBRACE_len;
        // Skip empty lines generated by directives
        if (((base > ctx.LBRACE_len &&
              buffer[base - ctx.LBRACE_len - 1] == '\n') ||
             base == ctx.LBRACE_len) &&
